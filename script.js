(function() {
  ...
  function navbarScrollEffect() {
    ...
    if (!navbar) return;
    var scrollClass = 'navbar-scrolled';
    var lastScroll = 0;
  ...
  function contactFormValidation() {
    ...
    form.addEventListener('submit', function(e) {
      ...
      if (isValid) {
        ...
        if (submitBtn) {
          ...
          submitBtn.textContent = submitBtn.getAttribute('data-loading') || 'Sending...';
        }
        // Simulate async submission
        setTimeout(function() {
          form.reset();
          fields.forEach(function(f) {
            f.classList.add(successClass);
          });
          if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.textContent = submitBtn.getAttribute('data-original') || 'Sent!';
            setTimeout(function() {
              submitBtn.textContent = submitBtn.getAttribute('data-original') || 'Submit';
              fields.forEach(function(f) {
                f.classList.remove(successClass);
              });
            }, 3000);
          }
        }, 1000);
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        errorMsg = field.getAttribute('data-error-minlength') || 'Minimum length is ' + field.getAttribute('minlength') + ' characters.';
      } else if (field.hasAttribute('maxlength') && value.length > parseInt(field.getAttribute('maxlength'), 10)) {
        errorMsg = field.getAttribute('data-error-maxlength') || 'Maximum length is ' + field.getAttribute('maxlength') + ' characters.';
      } else if (field.tagName === 'SELECT' && field.hasAttribute('required') && (value === '' || value === null)) {
        errorMsg = field.getAttribute('data-error-required') || 'Please select an option.';
      }
    ...
    function showFieldError(field, message) {
      ...
      var errorEl = field.parentNode.querySelector('.form-error-message');
      if (errorEl) {
  ...
  function scrollAnimations() {
    ...
    if (!document.getElementById('scroll-animations-style')) {
      ...
      style.textContent = '.fade-in-hidden{opacity:0;transform:translateY(20px);transition:opacity 0.6s ease-out,transform 0.6s ease-out;}.fade-in-visible{opacity:1;transform:translateY(0);}';
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '222ea07b-7d3e-47de-ad70-bad8d1b8f54a',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '222ea07b-7d3e-47de-ad70-bad8d1b8f54a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '222ea07b-7d3e-47de-ad70-bad8d1b8f54a';
